...
def bt_likelihood(scores):
    log_likelihood = 0
    # 按批次处理数据
    for i in tqdm(range(0, len(df), batch_size), desc="Processing batches"):  # 使用 tqdm 显示进度
        batch = df.iloc[i:i + batch_size]
        log_likelihood += bt_likelihood_batch(scores, batch)
    return log_likelihood

# 使用 BFGS 算法进行最大似然估计优化
result = minimize(bt_likelihood, initial_scores, method='BFGS', options={'disp': True, 'maxiter': 100})
