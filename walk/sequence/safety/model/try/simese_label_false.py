...
# false：预处理数据所占空间太过庞大，无法在本地运行
# --------------------------
# 全局配置：使用预处理数据（.npy）以加快训练速度
# --------------------------
...
# --------------------------
# 预处理函数：将所有图像转换为标签并保存为 .npy 文件
# --------------------------
def convert_image_to_label(np_img, color_map):
    """
    将 BGR 顺序的 numpy 图像转换为类别索引标签
    """
    h, w, _ = np_img.shape
    label_img = np.zeros((h, w), dtype=np.int64)
...
def preprocess_all_images(image_dir, save_dir, color_map):
    ...
    for filename in tqdm(image_files):
        ...
        # 保存文件名：保持原图文件名（去掉扩展名）+ _pred.npy
        base_name = os.path.splitext(filename)[0]
        save_path = os.path.join(save_dir, f"{base_name}_pred.npy")
        np.save(save_path, label_img)
    print("预处理完成！")
...
class SegmentationPairDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if use_preprocessed:
            # 预处理数据加载：直接加载 .npy 文件
            left_label_path = os.path.join(label_dir, f"{left_id}_pred.npy")
            right_label_path = os.path.join(label_dir, f"{right_id}_pred.npy")
            ...
            # 加载 numpy 数组，并转换为 tensor；确保形状为 (1, H, W)
            np_left = np.load(left_label_path)
            np_right = np.load(right_label_path)
...
class SiameseNetwork(nn.Module):
    def __init__(self):
        super().__init__()
        # 注意：输入通道从 3 改为 1，因为输入为语义标签图（单通道）
...
if __name__ == '__main__':
    ...
    # 日志与模型保存目录配置
    log_dir = "walk/deeplabv3-master/training_logs/siamese2"
