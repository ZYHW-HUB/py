...
# 读取评分数据（假设已存在的表 image_scores_safety_m）
image_scores_safety_m_df = pd.read_sql("SELECT image_id, score FROM image_scores_safety_m", con=conn)

# 将评分数据存储到字典中，方便快速查找
scores = dict(zip(image_scores_safety_m_df['image_id'], image_scores_safety_m_df['score']))
