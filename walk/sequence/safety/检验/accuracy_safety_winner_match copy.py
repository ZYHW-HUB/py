...
# 读取评分数据（假设已存在的表 elo_scores_safety）
elo_scores_safety_df = pd.read_sql("SELECT image_id, elo_score FROM elo_scores_safety", con=conn)

# 将评分数据存储到字典中，方便快速查找
scores = dict(zip(elo_scores_safety_df['image_id'], elo_scores_safety_df['elo_score']))
