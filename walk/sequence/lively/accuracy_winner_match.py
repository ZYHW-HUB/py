...
# 读取评分数据（假设已存在的表 image_scores_lively）
image_scores_lively_df = pd.read_sql("SELECT image_id, score FROM image_scores_lively", con=conn)

# 将评分数据存储到字典中，方便快速查找
scores = dict(zip(image_scores_lively_df['image_id'], image_scores_lively_df['score']))
