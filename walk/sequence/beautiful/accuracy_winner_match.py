...
# 读取评分数据（假设已存在的表 image_scores_beautiful）
image_scores_beautiful_df = pd.read_sql("SELECT image_id, score FROM image_scores_beautiful", con=conn)

# 将评分数据存储到字典中，方便快速查找
scores = dict(zip(image_scores_beautiful_df['image_id'], image_scores_beautiful_df['score']))

# 读取比赛数据
data = pd.read_sql("SELECT left_id, right_id, winner_label_encoded FROM pp2_combined WHERE category = 'beautiful'", con=conn)
